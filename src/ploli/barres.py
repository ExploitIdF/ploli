...
def plot_grouped_bars(
    ...
    for i in range(int(np.ceil(len(x_categories) / band_width))):
        rect = Rectangle((i * band_width-.5, 0), band_width/2, ylen, facecolor='#f0f0f0', zorder=0)
        ax.add_patch(rect)
    ...
    ax.set_yticklabels(group_heights.index, rotation=0, ha='right', fontsize=8)
    ...
    plt.tight_layout()
    plt.savefig(nom_fich, dpi=100, bbox_inches='tight')
    plt.show()
